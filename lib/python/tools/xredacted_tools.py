# tools/xredacted_tools.py
"""
xREDACTED CLI wrappers for Swarm agents.
...
def _run_clawnch(args: List[str], check: bool = True) -> Dict[str, Any]:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"xREDACTED command failed ({result.returncode}):\n"
            f"cmd: {' '.join(cmd)}\n"
...
if __name__ == "__main__":
    try:
        ...
        # tweet = post_tweet("Test from Swarm agent via xREDACTED 🦞")
        # print(tweet)
