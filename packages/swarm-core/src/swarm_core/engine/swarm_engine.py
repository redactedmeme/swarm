...
from swarm_core.engine.parallel_branch_engine import ParallelBranchEngine
try:  # optional: mem0-memory plugin dir is hyphenated, so import it by path
    ...
    def add_memory(*_a, **_kw):
        return None
# `services/telegram/` was renamed to the smolting bot directory long ago, so this
# import has been dead for as long as the rename. The bot dir is hyphenated and
# therefore not importable as a package; load it by path when present.
try:
    import sys as _sys
    from swarm_core.paths import repo_root as _rr
    _bot = _rr() / 'smolting-telegram-bot'
    if _bot.exists() and str(_bot) not in _sys.path:
        _sys.path.insert(0, str(_bot))
    from smolting_personality import SmoltingPersonality
except Exception:
    SmoltingPersonality = None
...
class SwarmEngine:
    ...
    async def _run_sevenfold_consensus(self, proposal: str) -> Dict:
        ...
        from swarm_core.engine.sevenfold_consensus import run_consensus
