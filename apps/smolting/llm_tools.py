...
TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "description": "Write a file in smolting's persistent workspace (survives restarts).",
        "parameters": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "workspace_read",
        "description": "Read a file back from smolting's persistent workspace.",
        "parameters": {
            "type": "object",
    ...
    {
        "name": "workspace_list",
        "description": "List a directory in smolting's persistent workspace.",
        "parameters": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Open a URL in smolting's workspace browser and return the readable page text.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {"type": "string"},
...
async def exec_write_lore(content: str, category: str = "lore", title: str = None) -> dict:
    ...
    if category not in valid_categories:
        category = "lore"
    try:
        from swarm_core.lore_vault import add_entry
        entry_id = add_entry(
            content=content[:300],
            category=category,
            title=title,
...
async def exec_workspace_write(path: str, content: str, append: bool = False) -> dict:
    if _WS is None:
        return {"success": False, "error": "workspace unavailable"}
    r = _ws_ok(_WS.write(path, content, append=bool(append)))
    _log_tool_call("workspace_write", {"path": path, "append": bool(append)}, r)
    return r
