# redactedbuilder-telegram-bot/swarm_inbox.py
"""
...
def _redis_load_pending(r, agent: str) -> list[dict]:
    ...
    (Broadcasts fanned into a per-agent set are zrem'd from swarm:pending:all on
    completion, not from the per-agent set — this reaps those tombstones.)
    """
    msgs = []
