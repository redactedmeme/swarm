# smolting-telegram-bot/python/vault_io.py  (bundled — keep in sync with vault/vault_io.py)
"""
...
# Resolve vault root: env var > bundled vault/ next to main.py > repo-root vault/
_HERE = Path(__file__).resolve().parent          # smolting-telegram-bot/python/
_SERVICE_ROOT = _HERE.parent                     # smolting-telegram-bot/
_VAULT_ROOT = (
    Path(os.environ["VAULT_PATH"])               if "VAULT_PATH" in os.environ          else
    _SERVICE_ROOT / "vault"                      if (_SERVICE_ROOT / "vault").exists()  else
    _SERVICE_ROOT.parent / "vault"               if (_SERVICE_ROOT.parent / "vault").exists() else
    _SERVICE_ROOT / "vault"                      # fallback — ops fail gracefully
)
