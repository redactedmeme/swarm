    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        {isLoading ? (
          ...
          <>
            {/* Spatial canvas */}
            <div className="relative mb-6 bg-card/30 border border-border rounded-xl overflow-hidden" style={{ minHeight: 320 }}>
              <SpatialCanvas agents={agents} selected={selected} onSelect={setSelected} />
            </div>
...
// ── Spatial canvas with draggable nodes ──────────────────────────────────────
...
function SpatialCanvas({ agents, selected, onSelect }: SpatialCanvasProps) {
  const canvasW = 600
  const canvasH = 300
  ...
  return (
    <svg viewBox={`0 0 ${canvasW} ${canvasH}`} className="w-full h-full" style={{ minHeight: 240 }}>
      ...
      {/* Connection lines from core agent */}
      {agents.map((agent) => {
      ...
      {/* Agent nodes */}
      {agents.map((agent) => {
        const pos = INITIAL_POSITIONS[agent.id] ?? { x: Math.random(), y: Math.random() }
        ...
        {selected && (
          ...
          >
            <div className="mt-3 pt-3 border-t border-border/50 flex items-center justify-between">
              ...
              {agent.last_seen && (
                <span className="text-xs text-muted-foreground">Last seen {agent.last_seen}</span>
              )}
            </div>
          </motion.div>
        )}
